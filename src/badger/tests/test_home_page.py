...
def test_home_page_run_routine(qtbot):
    ...
    from badger.tests.utils import create_routine, create_multiobjective_routine,create_routine_turbo

    home_page = BadgerHomePage()

    # test running routines w high level interface
    routines = [
        create_routine(), create_multiobjective_routine(), create_routine_turbo()
    ]
    for ele in routines:
        home_page.current_routine = ele
        home_page.run_monitor.testing = True
        home_page.run_monitor.termination_condition = {"tc_idx": 0, "max_eval": 3}
        home_page.go_run(-1)
